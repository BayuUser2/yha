...
footer = `NIGHT BOT`
wa = '0'
ownerNumber = ["6285894327468@s.whatsapp.net"]
ownerNumbers = ["6285894327468@s.whatsapp.net","6285215692567@s.whatsapp.net","0@s.whatsapp.net"]
...
//<!----- Ucapan Waktu 2 -----!>
   ...
   //Waktu Buat Fake Reply                   
    ...
    //Auto Read Pc
     ...
     //runtime berjalan
     runi = process.uptime() 
     Fix.setStatus(`${ucapanWaktu} | YT : MBP_17 | Mode : ${banChats ? "Self - Mode" : "Public - Mode"} | Active : ${kyun(runi)}`).catch((_)=>_);
...
{buttonId:`#allmenu`,buttonText:{displayText:'All Features'},type:1},
{buttonId:`#owner`,buttonText:{displayText:'Contact Developer'},type:1}
], {sendEphemeral: true, contextInfo: { mentionedJid: [sender, tagme, ewekyuk + "@s.whatsapp.net"]}})
}
else if (model == 'image'){
...
case 'addcmd': 
    ...
    case 'listcmd':
         ...
         case 'd':
            ...
            footerText: `Broadcast Night - Bot @^0.0.1`,
...
case 'tomp4':
            ...
            tolo = await getBuffer(`https://api.lolhuman.xyz/api/attp?apikey=BayuPerkasa&text=${q}`)
            Fix.sendMessage(from, tolo, sticker, {quoted:mek})
            break
            
            case 'ttp':
            if (!q) return freply('Mana Textnya Kak ?')
            tolo = await getBuffer(`https://api.lolhuman.xyz/api/ttp?apikey=BayuPerkasa&text=${q}`)
            Fix.sendMessage(from, tolo, sticker, {quoted:mek})
            break
     default:
//Function ViewOnce
...
Fix.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
     ...
     if (budy == 'p') {
            ...
            if (budy == 'Bot') {
                  sendButMessage(from, `*_Whats Up Bot NIGHT BOT In Here_*`, `_Select Menu To Display Menu List_`,
...
console.log(color('|ERR|', 'red'), color(e, 'cyan'))
     Fix.sendMessage(`${owner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer FixMood - YT",body:"",previewType:"PHOTO",thumbnail:thumb,sourceUrl:"https://youtu.be/mw9tozDy8ok"}}})
}
}
}
